...
void inverse(Matrix invest,Matrix &inverse)
{
    if(determinant(invest)!=0){
        Matrix cof,tran;
        cofactors(invest,cof);
        transpose(cof,tran);
        for (int i = 0; i < invest.size(); i++)
        {
            for (int j = 0; j < invest.at(0).size(); j++)
            {
                inverse.at(i).at(j)= tran.at(i).at(j)/determinant(invest);
            }
